...
### 実行結果（Windows / Julia 1.12.7、合成データ・別試験）
`dpd_model_from_data.jl` を実機（Windows, Julia 1.12.7）で実行した結果：

| モデル | 係数数 | NMSE |
|---|---|---|
| **MP 7次/メモリー深さ5** | 24 | **−34.71 dB** |
| GMP（交差項） | 42 | −34.74 dB |

- ILA による DPD 係数抽出：MP-DPD 24 係数、**推定小信号利得 |G| = 1.849（5.3 dB）**。
- **判断**：GMP は MP 比 +18 係数に対し NMSE 改善が約 0.03 dB に留まるため、現段階の第一候補は **MP 7次/メモリー深さ5**。
- ※ これは合成データによる追加検証（別試験）。既存の EVM 2.04%/ACLR 40.3dB 原理検証（`DPD_メモリー効果_シミュレーション結果.md`）とは別。実機 I/Q 取得後に DPD OFF/ON で NMSE・EVM・ACLR を最終評価する。
- 係数数（MP 24＝奇数4次×メモリ6タップ、GMP 42＝+交差項18）はスクリプトの基底構成と一致（動作確認済み）。

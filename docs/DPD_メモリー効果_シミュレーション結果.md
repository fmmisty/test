...
| **MP DPD（7次・メモリー深さ5）** | **2.00** | **40.2** | ✓ 合格 |
| **GMP DPD（交差項追加）** | **1.98** | **40.6** | ✓ 合格(最良) |

> 実装方針（FPGA）: **まず 7次・メモリー深さ5 の MP** を作り、**ACLR が不足した場合のみ GMP の交差項を追加**する段階構成が現実的。上表でも MP 7次/深さ5 で 64QAM を満たし、GMP の上乗せは小。
